...
export default function DevBanner () {
    if (process.env.NEXT_PUBLIC_APP_DEV_MODE == 'false') return (<></>)
        ...
        <div className="dev-banner-info absolute z-40 top-auto bottom-0 container-full-width py-4 px-3 overflow-hidden">
            <div className="animate-fadeupwards flex flex-row content-max px-6 py-4 bg-gray-900 rounded-2xl shadow-md">
                <div className="flex flex-col mr-4 my-auto">
                    ...
                    <p className="animate-fadeupwards mt-1 font-mono font-normal text-left text-3xs tracking-normal text-light-grayed-out">buildinfo: proj_ver {packageInfo.version}, provider_ver {packageInfo.dependencies['next']}, renderer_ver {packageInfo.dependencies['react']}, deps [{Object.keys(packageInfo.dependencies).join(', ')}], dev_deps [{Object.keys(packageInfo.devDependencies).join(', ')}], mode {process.env.NEXT_PUBLIC_APP_DEV_MODE}</p>
                </div>
                <div
